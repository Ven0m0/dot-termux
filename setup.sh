...
find_files() {
  # find_files -type f -name ext -x -d maxdepth -0 -- path/to/dir
  local findopts=() path=. null=0
  while [[ $# -gt 0 ]]; do
    case $1 in
      -type) findopts+=("-type" "$2"); shift 2;;
      -name) findopts+=("-name" "*.$2"); shift 2;;
      -x) findopts+=("-executable"); shift;;
      -d) findopts+=("-maxdepth" "$2"); shift 2;;
      -0) null=1; shift;;
      --) shift; break;;
      -*) shift;;
      *) path="$1"; shift;;
  ...
  ((null)) && findopts+=(-print0) || findopts+=(-print)
  find "$path" "${findopts[@]}" 2>/dev/null || :
}
