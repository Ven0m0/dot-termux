...
if [[ "${BASH_SOURCE[0]:-}" != "$0" ]]; then
  ...
  if [[ -n "${BASH_SOURCE[0]:-}" && "${BASH_SOURCE[0]}" != "-" ]]; then
    cd -P -- "$(dirname -- "${BASH_SOURCE[0]}")" &>/dev/null || true
  fi
fi
...
install_jetbrains_mono() {
  ...
  if curl -sL "$url" -o "$temp_zip"; then
    unzip -jo "$temp_zip" "fonts/ttf/JetBrainsMono-Regular.ttf" -d "$font_dir" &>/dev/null &&
...
setup_revanced_tools() {
  ...
  { log "Installing X-CMD..."; curl -fsSL https://get.x-cmd.com | bash &>/dev/null; log "X-CMD installation complete"; } &
  { log "Installing SOAR..."; curl -fsSL https://soar.qaidvoid.dev/install.sh | sh &>/dev/null; log "SOAR installation complete"; } &
...
optimize_zsh() {
  ...
  zsh -c 'zmodload zsh/zcompiler; zcompile ~/.zshrc; zcompile ~/.zshenv' &>/dev/null
