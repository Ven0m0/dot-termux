...
get_output_path(){
  ...
  [[ -n $OUTPUT_DIR ]] && local dir=$OUTPUT_DIR || local dir="${src%/*}"
...
# ---- Image Optimization ----
optimize_png(){
  local src=$1 out=$2 tmp="${TEMP_DIR}/$(basename "$out").tmp"
  local orig=$(get_size "$src") success=0
  
  cp "$src" "$tmp"
  if has oxipng; then
    oxipng -o6 --strip safe -q "$tmp" &>/dev/null && success=1
    [[ $LOSSLESS -eq 0 ]] && has pngquant && pngquant --quality=65-"$QUALITY" --strip --speed 1 -f "$tmp" -o "${tmp}.2" &>/dev/null && mv "${tmp}.2" "$tmp" || :
  elif has optipng; then
    optipng -o7 -strip all -quiet "$tmp" &>/dev/null && success=1
    [[ $LOSSLESS -eq 0 ]] && has pngquant && pngquant --quality=65-"$QUALITY" --strip --speed 1 -f "$src" -o "$tmp" &>/dev/null || :
  fi
  has flaca && flaca --no-symlinks --preserve-times "$tmp" &>/dev/null || :
...
optimize_jpeg(){
  ...
  has flaca && flaca --no-symlinks --preserve-times "$tmp" &>/dev/null || :
  has rimage && rimage -i "$tmp" -o "${tmp}.r" &>/dev/null && mv -f "${tmp}.r" "$tmp" || :
...
select_image_target_format(){
  ...
  for c in "${IMAGE_CODEC_PRIORITY[@]}"; do
    case "$c" in
      webp) has cwebp && { echo "webp"; return; } ;;
      avif) has avifenc && { echo "avif"; return; } ;;
      jxl) has cjxl && { echo "jxl"; return; } ;;
...
optimize_image(){
  ...
  if [[ $fmt != "$ext" ]]; then
    ...
    case "$fmt" in
      webp)
        has cwebp && cwebp -q "$QUALITY" -m 6 -mt -af "$src" -o "$tmp" &>/dev/null && success=1
        ;;
      avif)
        has avifenc && avifenc -s "$AVIF_SPEED" -j "$NPROC" --min 0 --max "$AVIF_QUAL" "$src" "$tmp" &>/dev/null && success=1
        ;;
      jxl)
        if has cjxl; then
    ...
    case "$ext" in
      ...
      gif) has gifsicle && gifsicle -O3 "$src" -o "$out" &>/dev/null || cp "$src" "$out" ;;
      svg) has svgo && svgo -i "$src" -o "$out" &>/dev/null || has scour && scour -i "$src" -o "$out" --enable-id-stripping &>/dev/null || cp "$src" "$out" ;;
      webp) has cwebp && cwebp -q "$QUALITY" -m 6 -mt "$src" -o "$out" &>/dev/null || cp "$src" "$out" ;;
      avif) has avifenc && avifenc -s "$AVIF_SPEED" -j "$NPROC" --min 0 --max "$AVIF_QUAL" "$src" "$out" &>/dev/null || cp "$src" "$out" ;;
      jxl) has cjxl && cjxl "$src" "$out" -d 0 -e 7 &>/dev/null || cp "$src" "$out" ;;
...
tui_select(){
  local dir=$1
  has sk || has fzf || err "TUI requires sk or fzf"
