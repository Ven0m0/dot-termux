...
check_deps() {
  ...
  ((missing)) && { echo "Install via: pkg install rust imagemagick ffmpeg flac" >&2; exit 1; }
...
process_file() {
  ...
  case "${ext,,}" in
    jpg|jpeg|webp|avif)
      rimage -q "$IMG_QUALITY" "$file" -o "$out_file" >/dev/null 2>&1 || true
      ;;
    png)
      oxipng -o max --strip safe "$file" --out "$out_file" >/dev/null 2>&1 || true
      ;;
    mp4|mov|mkv|webm)
      ffmpeg -i "$file" -c:v libx265 -preset medium -crf "$VID_CRF" \
        -c:a copy -tag:v hvc1 -y "$out_file" >/dev/null 2>&1 || true
      ;;
    flac)
      # flaca is for FLAC audio, not images.
      cp "$file" "$out_file" >/dev/null 2>&1 || true
      flaca --best "$out_file" >/dev/null 2>&1 || true
      ;;
    *)
