    ...
    for f in "$d"/. "$d"/..?* "$d"/*; do
      ...
      if ((show_path)); then
        ...
        if ((allow_dupe)); then
          ...
          [[ -z ${seen["$base"]:-} ]] && { out+=("$base"); seen["$base"]=1; }
        fi
      fi
    done
  done
...
E
  ...
  local topic=$1; [[ -n ${topic:-} ]] && shift || :
  if [[ -z $topic ]]; then
    has fzf || return 1
    topic=$(cat "$CHT_SH_LIST_CACHE" \
      | fzf --reverse --height 75% --border -m --ansi --nth 2..,.. \
...
ln2(){
  local -a all=("$@")
  [[ " ${all[*]} " =~ \ -(h|-help)\  ]] && { _ln2_usage; return 0; }
  ((${#all[@]}>=3)) || { _ln2_usage; return 1; }
  local operand2=${all[-1]} op=${all[-2]} operand1=${all[-3]}
  local -a opts=("${all[@]:0:${#all[@]}-3}")
  [[ $operand1 == -* ]] && { _ln2_usage; return 1; }
  local target link
  case $op in
    '>') target=$operand2; link=$operand1;;
    '<') target=$operand1; link=$operand2;;
    *) _ln2_usage; return 1;;
  esac
  ln "${opts[@]}" "$target" "$link"
}
